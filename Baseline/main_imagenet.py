...
# Training settings
parser = argparse.ArgumentParser(description='Baseline PyTorch implementation (ImageNet dataset)')

parser.add_argument('-n', '--network', default='ResNet',
					help='Neural network model to use: Choose ResNet or VGG')
parser.add_argument('-l', '--layers', type=int, default=18,
...
parser.add_argument('-nw', '--number_workers', type=int, default=8,
...
parser.add_argument('-d', '--distributed', type=str2bool, nargs='?', const=True, default=False,
...
if __name__ == '__main__':
	...
	if args.network.lower() in ('resnet', 'vgg'):
		args.networkCfg = f'ImageNet.{args.network}-{args.layers}{bn}.Baseline.W32.A32.G32.{args.optimizer}.LR{args.learning_rate}'
	elif args.network.lower() == 'alexnet':
		args.networkCfg = f'ImageNet.{args.network}.Baseline.W32.A32.G32.{args.optimizer}.LR{args.learning_rate}'
	...
	if args.load_checkpoint:
		...
		if args.network.lower() == 'resnet':
			network = ResNet_imagenet(layers=args.layers)
		elif args.network.lower() == 'vgg' and args.layers in (11, 13, 16, 19):
			network = VGG(model_name[args.layers], num_classes=1000, batch_norm=args.batch_norm)
		elif args.network.lower() == 'alexnet':
			network = AlexNet()
		else:
