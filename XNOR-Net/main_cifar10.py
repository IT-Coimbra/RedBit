...
def ParseArgs():
	parser = argparse.ArgumentParser(description='XNOR-Net Pytorch CIFAR-10 Implementation.')
...
if __name__ == '__main__':
	...
	if args.network.lower() == 'resnet':
		network = ResNet_cifar(layers=args.layers, insert_relu=args.insert_relu, binarize_activations=args.binarize_activations).cuda()
	elif args.network.lower() == 'vgg' and args.layers in (11, 13, 16, 19):
		network = VGG(model_name[args.layers], insert_relu=args.insert_relu, binarize_activations=args.binarize_activations, batch_norm=args.batch_norm).cuda()
	else:
	...
	if args.binarize_activations:
		if args.insert_relu:
			args.networkCfg = f'CIFAR-10.{args.network}-{args.layers}{bn}.XNOR-Net.ReLU.{args.optimizer}'
		else:
			args.networkCfg = f'CIFAR-10.{args.network}-{args.layers}{bn}.XNOR-Net.{args.optimizer}'
	else:
		if args.insert_relu:
			args.networkCfg = f'CIFAR-10.{args.network}-{args.layers}{bn}.BWN.ReLU.{args.optimizer}'
		else:
			args.networkCfg = f'CIFAR-10.{args.network}-{args.layers}{bn}.BWN.{args.optimizer}'
