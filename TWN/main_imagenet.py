...
from tools import RunManager_i, TernarizeOp, clear, str2bool, accuracy, renameBestModel_i


def ParseArgs():
	parser = argparse.ArgumentParser(description='Ternary-Weights-Network PyTorch ImageNet.')
...
if __name__ == '__main__':
	...
	if args.ternarize:
		...
		if args.network.lower() in ('resnet', 'vgg'):
			args.networkCfg = f'ImageNet.{args.network}-{args.layers}{bn}.TWN.{args.optimizer}.LR{args.learning_rate}'
			
		elif args.network.lower() == 'alexnet':
			args.networkCfg = f'ImageNet.{args.network}.TWN.{args.optimizer}.LR{args.learning_rate}'
	else:
		ternarize_op = None
		if args.network.lower() in ('resnet', 'vgg'):
			args.networkCfg = f'ImageNet.{args.network}-{args.layers}{bn}.FPWN.{args.optimizer}.LR{args.learning_rate}'
		elif args.network.lower() == 'alexnet':
			args.networkCfg = f'ImageNet.{args.network}.FPWN.{args.optimizer}.LR{args.learning_rate}'
